...
if (!input) {
  console.error('Usage: node scripts/k6-report.mjs <summary.json> [report.html]');
  process.exit(1);
}
...
const breached = Object.values(metrics).reduce(
  (n, item) => n + Object.values(item.thresholds ?? {}).filter((v) => v === false).length,
  0,
);
const rows = Object.entries(metrics)
  .filter(([name]) => /^(http_req_|grpc_req_|iteration_duration|rest_operation_duration)/.test(name))
  .map(([name, item]) => metricRow(name, item))
  .join('');
const checkRows = Object.entries(summary.root_group?.checks ?? {})
  .map(
    ([name, item]) =>
      `<tr><td>${esc(name)}</td><td>${item.passes ?? 0}</td><td>${item.fails ?? 0}</td><td>${item.fails ? '✗ Failed' : '✓ Passed'}</td></tr>`,
  )
  .join('');
...
function metricRow(name, v) {
  return `<tr><td>${esc(name)}</td><td>${fmt(v.count)}</td><td>${rate(v.rate)}</td><td>${fmt(v.avg)}</td><td>${fmt(v.max)}</td><td>${fmt(v.med)}</td><td>${fmt(v.min)}</td><td>${fmt(v['p(90)'])}</td><td>${fmt(v['p(95)'])}</td></tr>`;
}
function fmt(v) {
  return v === undefined || v === null ? '-' : typeof v === 'number' ? Number(v.toFixed(2)).toLocaleString() : esc(v);
}
function rate(v) {
